...
test("server-renders the Cadence learning studio", async () => {
  ...
  assert.match(html, /Connect your Casio CT-S1/);
  assert.match(html, /Music microscope/);
  assert.match(html, /Keyboard geography/);
  assert.doesNotMatch(html, /codex-preview|react-loading-skeleton|Your site is taking shape/i);
});

test("keeps the AI boundary isolated and local persistence explicit", async () => {
  const [page, coach, hosting] = await Promise.all([
  ...
  assert.match(page, /cadence\.progress\.v1/);
